...
def add_person(firstname, lastname, birth, tel, adresse, mail, id_bien, mode_regl, date_regl, rest_payer, divers, list_com, real_divers):
	with open('test_database.json') as f:
		data = json.load(f)
		f.close()
		add = {"nom": firstname,
				...
				"Adresse": adresse,
				"Mail": mail,
				"Identifiant Bien Etre": id_bien,
				"mode de reglement" : mode_regl,
				"date de reglement" : date_regl,
				"reste a payer" : rest_payer,
			    "divers infos": divers,
			    "list com": list_com,
			    "divers": real_divers}
...
def modif_person(firstname, lastname, birth, tel, adresse, mail, id_bien, mode_regl, date_regl, rest_payer, divers, list_com, real_divers):
	with open('test_database.json') as f:
		data = json.load(f)
		f.close()
		add = {"nom": firstname,
...
def delete_person(input_nom):
	with open('test_database.json') as f:
		...
		for name in data['people']:
			prenom = name['prenom'].lower()
			nom = name['nom'].lower()
			if prenom == input_nom.lower() or nom == input_nom.lower():
				pass
			else:
